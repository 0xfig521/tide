...
export function Hero() {
  ...
  return (
    <section className="relative pt-32 pb-24 min-h-[100dvh] flex items-center overflow-hidden">
      ...
      <div className="absolute top-1/4 left-1/4 -translate-x-1/2 -translate-y-1/2 w-[40vw] h-[40vw] rounded-full bg-brand-cyan/15 aura-glow animate-pulse-slow pointer-events-none" />
      <div className="absolute bottom-1/4 right-1/4 translate-x-1/2 translate-y-1/2 w-[45vw] h-[45vw] rounded-full bg-brand-violet/15 aura-glow animate-pulse-slow pointer-events-none" style={{ animationDelay: "-3s" }} />
      <div className="absolute top-1/2 left-2/3 -translate-x-1/2 -translate-y-1/2 w-[35vw] h-[35vw] rounded-full bg-brand-pink/10 aura-glow animate-pulse-slow pointer-events-none" style={{ animationDelay: "-6s" }} />
      ...
      <div className="max-w-6xl mx-auto px-6 w-full relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          ...
          <div className="lg:col-span-6 text-center lg:text-left">
            ...
            {/* Headline */}
            <h1 className="text-5xl sm:text-6xl font-extrabold tracking-tight leading-[1.08] text-white">
              {t.hero.headlinePart1}{" "}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-cyan via-brand-violet to-brand-pink filter drop-shadow-[0_2px_20px_rgba(6,182,212,0.15)] block mt-1">
                {t.hero.headlineAccent}
              </span>
            </h1>

            {/* Description */}
            <p className="mt-8 text-lg text-terminal-dim leading-relaxed max-w-[55ch] mx-auto lg:mx-0 font-medium">
              {t.hero.description}
            </p>
            ...
            <div className="mt-10 flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4">
              <a
                href="#install"
                className="group relative inline-flex items-center justify-center px-6 py-3.5 w-full sm:w-auto font-sans text-sm font-bold text-black bg-white rounded-full overflow-hidden hover:scale-[1.02] active:scale-[0.98] transition-all duration-300 shadow-xl shadow-white/5"
              >
              ...
              <a
                ...
                className="inline-flex items-center justify-center px-6 py-3.5 w-full sm:w-auto font-sans text-sm font-semibold text-white/95 rounded-full border border-white/10 hover:border-white/20 bg-white/5 hover:bg-white/10 active:scale-[0.98] transition-all duration-300 backdrop-blur-md"
              >
                {t.hero.viewOnGitHub}
              </a>
            </div>
          ...
          <div className="lg:col-span-6 w-full">
            <div className="relative">
              ...
              <div className="relative rounded-2xl glass-panel overflow-hidden border border-white/10 shadow-2xl bg-slate-950/80">
                ...
                {/* Terminal Body */}
                <div className="p-6 font-mono text-sm leading-relaxed min-h-[380px] flex flex-col justify-between">
                  <div>
                    {/* Shell Prompt */}
                    ...
                    {showJson && (
                      <pre
                        ref={jsonRef}
                        className={`mt-4 text-xs overflow-hidden transition-all duration-500 rounded bg-black/40 border border-white/5 p-4 text-white/80 ${
                          glowActive
                    ...
                    {/* Transform Glow Line */}
                    {glowActive && (
                      <div className="h-0.5 bg-gradient-to-r from-transparent via-terminal-green to-transparent w-full blur-[1px] animate-pulse" />
                    )}
                  </div>

                  {/* Render Visual RSS Cards */}
                  {showCards && (
                    <div ref={cardsContainerRef} className="mt-4 flex flex-col gap-3">
                      {MOCK_ARTICLES.map((article) => (
                        ...
                        >
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex items-center gap-2">
                              <span className="text-[10px] font-semibold uppercase tracking-wider text-brand-cyan px-2 py-0.5 rounded bg-brand-cyan/10 border border-brand-cyan/15">
                                {article.category}
                              </span>
